...
Dictionary and text input routines for the mudflat event coder
...
GitHub repository: https://github.com/philipschrodt/mudflat
...
18-Mar-17:	Initial modifications from PETRreader.py
...
def parse_Config(config_path):
    """
    Parse mf_globals.ConfigFileName. The file should be ; the default is PETR_config.ini
...
#	print "pc",mf_globals.ConfigFileName
    confdat = parser.read(config_path)
    if len(confdat) == 0:
        print(
            "\aError: Could not find the config file:",
            mf_globals.ConfigFileName)
        print("Terminating program")
        sys.exit()

    try:
        mf_globals.VerbFileName = parser.get('Dictionaries', 'verbfile_name')
        mf_globals.AgentFileName = parser.get(
            'Dictionaries',
            'agentfile_name')
#		print "pc",mf_globals.AgentFileName
        mf_globals.DiscardFileName = parser.get(
            'Dictionaries',
            'discardfile_name')

        filestring = parser.get('Dictionaries', 'actorfile_list')
        mf_globals.ActorFileList = filestring.split(', ')

        # otherwise this was set in command line
        if len(mf_globals.TextFileList) == 0:
            if parser.has_option('Options', 'textfile_list'):  # takes priority
                filestring = parser.get('Options', 'textfile_list')
                mf_globals.TextFileList = filestring.split(', ')
            else:
                ...
                while len(line) > 0:   # go through the entire file
                    ...
                    line = line.strip()
                    if len(line) > 0:
                        mf_globals.TextFileList.append(line)
                    line = fpar.readline()
                fpar.close()


        if parser.has_option('Dictionaries', 'issuefile_name'):
            mf_globals.IssueFileName = parser.get(
                'Dictionaries',
                'issuefile_name')

        if parser.has_option('Options', 'new_actor_length'):
            try:
                mf_globals.NewActorLength = parser.getint(
                    'Options',
                    'new_actor_length')
            except ValueError:
                print(
                ...
                raise
        print("new_actor_length =", mf_globals.NewActorLength)

        mf_globals.StoponError    = get_config_boolean('stop_on_error')
        mf_globals.WriteActorRoot = get_config_boolean('write_actor_root')
        mf_globals.WriteActorText = get_config_boolean('write_actor_text')
        mf_globals.WriteEventText = get_config_boolean('write_event_text')
        mf_globals.NullVerbs  = get_config_boolean('null_verbs')
        mf_globals.NullActors = get_config_boolean('null_actors')

        if parser.has_option(
                'Options', 'require_dyad'):  # this one defaults to True
            mf_globals.RequireDyad = get_config_boolean('require_dyad')
        else:
            mf_globals.RequireDyad = True

        # otherwise this was set in command line
        if len(mf_globals.EventFileName) == 0:
            mf_globals.EventFileName = parser.get('Options', 'eventfile_name')

        mf_globals.CodeBySentence = parser.has_option(
            'Options',
            'code_by_sentence')
        print("code-by-sentence", mf_globals.CodeBySentence)

        mf_globals.PauseBySentence = parser.has_option(
            'Options',
            'pause_by_sentence')
        print("pause_by_sentence", mf_globals.PauseBySentence)

        mf_globals.PauseByStory = parser.has_option(
            'Options',
            'pause_by_story')
        print("pause_by_story", mf_globals.PauseByStory)

        try:
            if parser.has_option('Options', 'comma_min'):
                mf_globals.CommaMin = parser.getint('Options', 'comma_min')
            elif parser.has_option('Options', 'comma_max'):
                mf_globals.CommaMax = parser.getint('Options', 'comma_max')
            elif parser.has_option('Options', 'comma_bmin'):
                mf_globals.CommaBMin = parser.getint('Options', 'comma_bmin')
            elif parser.has_option('Options', 'comma_bmax'):
                mf_globals.CommaBMax = parser.getint('Options', 'comma_bmax')
            elif parser.has_option('Options', 'comma_emin'):
                mf_globals.CommaEMin = parser.getint('Options', 'comma_emin')
            elif parser.has_option('Options', 'comma_emax'):
                mf_globals.CommaEMax = parser.getint('Options', 'comma_emax')
        except ValueError:
            print(
        ...
        print("Initial :", end=' ')
        if mf_globals.CommaBMax == 0:
            print("deactivated")
        else:
            print(
                "min =",
                mf_globals.CommaBMin,
                "   max =",
                mf_globals.CommaBMax)
        print("Internal:", end=' ')
        if mf_globals.CommaMax == 0:
            print("deactivated")
        else:
            print(
                "min =",
                ...
                mf_globals.CommaMax)
        print("Terminal:", end=' ')
        if mf_globals.CommaEMax == 0:
            print("deactivated")
        else:
            print(
                "min =",
                mf_globals.CommaEMin,
                "   max =",
                mf_globals.CommaEMax)
...
#	print "PTR-1:", theline,
    ...
    if ' ' not in theline:  # theline only contains a keyword
        mf_globals.AttributeList = theline[1:-2]
#		print "PTR-1.1:", mf_globals.AttributeList
        return
    pline = theline[1:].partition(' ')  # skip '<'
    mf_globals.AttributeList = [pline[0]]
    theline = pline[2]
    while ('=' in theline):  # get the field and content pairs
        pline = theline.partition('=')
        mf_globals.AttributeList.append(pline[0].strip())
        theline = pline[2]
        pline = theline.partition('"')
        if pline[2][0] == '"':   # twice-double-quote
            pline = pline[2][1:].partition('"')
            mf_globals.AttributeList.append('"' + pline[0] + '"')
            theline = pline[2][1:]
        else:
            pline = pline[2].partition('"')
            mf_globals.AttributeList.append(pline[0].strip())
            theline = pline[2]
#	print "PTR-2:", mf_globals.AttributeList
...
# routine); if an error needs to be raised, use get_attribute()
    if (targattr in mf_globals.AttributeList):
        return (
            mf_globals.AttributeList[
                mf_globals.AttributeList.index(targattr) + 1]
        )
    else:
        return ""
...
def read_discard_list(discard_path):
    ...
    logger.info("Reading " + mf_globals.DiscardFileName)
    ...
    while len(line) > 0:  # loop through the file
        ...
        targ = targ[1:]
        list = mf_globals.DiscardList.setdefault(prev, {})
        while targ != []:
...
def read_issue_list(issue_path):
    ...
    File is stored in mf_globals.IssueList as a list of tuples (string, index) where
    index refers to the location of the code in mf_globals.IssueCodes. The coding is done
    in check_issues()
    ...
    """
    mf_globals.IssueList = {}
    logger = logging.getLogger('petr_log')
    logger.info("Reading " + mf_globals.IssueFileName)
    open_FIN(issue_path, "issues")

    mf_globals.IssueCodes.append('~')  # initialize the ignore codes
    mf_globals.IssueCodes.append('~~')
    ...
    while len(line) > 0:  # loop through the file
        ...
        if line[0] == '~':  # ignore codes are only partially implemented
            ...
            if code in mf_globals.IssueCodes:
                codeindex = mf_globals.IssueCodes.index(code)
            else:
                mf_globals.IssueCodes.append(code)
                codeindex = len(mf_globals.IssueCodes) - 1
	...
	ka = 0
	while ka < 128 :
		print mf_globals.IssueList[ka],mf_globals.IssueCodes[mf_globals.IssueList[ka][1]]
		ka += 1
	"""
...
def read_verb_dictionary(verb_path):
    ...
    logger = logging.getLogger('petr_log')
    logger.info("Reading " + mf_globals.VerbFileName)
    ...
    def resolve_synset(line):
        ...
        segs = line.split()
        #print(line)
        syns = list(filter(lambda a: a.startswith('&'), segs))
        lines = []
        if syns:
    ...
    for line in file:
        ...
        if line.startswith("---"):
            ...
            for pat in resolve_synset(pattern):
                ...
                path =  mf_globals.VerbDict['phrases'].setdefault(block_meaning,{})
                if not pre == ([],[]):
                    if pre[0]:
                        count = 1
            ...
            answer = segs[1].split()
            ev2 = pat
            path = mf_globals.VerbDict['transformations']
            while len(ev2) > 1:
                source = ev2[0]
                verb = reduce(lambda a,b : a + b ,
                    map(lambda c: utilities.convert_code(mf_globals.VerbDict['verbs'][c]['#']['#']['code'])[0] if not c== "Q" else -1,
                    ev2[-1].split("_")), 0)
...
#                print(stem)
            ...
            for w in words:
                ...
                path = mf_globals.VerbDict['verbs']
    ...
    #print(sorted(mf_globals.VerbDict['phrases'].keys()))
    #print(mf_globals.VerbDict.__sizeof__())
    #print(mf_globals.VerbDict['phrases'].__sizeof__())
    #exit()
    #for root , k in sorted(mf_globals.VerbDict['verbs'].items()):
    #    print(root)
    #    print("\t",k)
...
def _read_verb_dictionary(verb_path):
    ...
    def make_phrase_list(thepat):
        ...
        while ka < len(phlist):
            if len(phlist[ka]) > 0:
                if (phlist[ka][0] == '&') and (
                        phlist[ka] not in mf_globals.VerbDict):
                    print("WTF", phlist[ka])
                    print(sorted(mf_globals.VerbDict.keys()))
                    exit()
...
def show_verb_dictionary(filename=''):
    ...
    if len(filename) > 0:
        ...
        fout.write('Run time: ' + mf_globals.RunTimeString + '\n')

        for locword, loclist in mf_globals.VerbDict.items():
    ...
    while len(line) > 0:
        if line[0] == '[':  # Date
            ...
            if line[0] == '+':  # Synonym
                ...
                if mf_globals.WriteActorRoot and len(current_acts) > 0:  # store the root phrase if we're only to use it
                    datelist.append(current_acts[0])
                for targ in current_acts:
                    list = mf_globals.ActorDict
                    while targ != []:
                        if targ[0] in [' ', '']:
                            targ = targ[1:]
                            continue
...
#		print subdict[marker]
    ...
    # note that this will be ignored if there are no errors
    logger = logging.getLogger('petr_log')
    logger.info("Reading " + mf_globals.AgentFileName + "\n")
