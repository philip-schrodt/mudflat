...
def parse_Config(config_path):
    """
    Parse globals.ConfigFileName. The file should be ; the default is PETR_config.ini
...
#	print "pc",globals.ConfigFileName
    ...
    try:
        globals.VerbFileName = parser.get('Dictionaries', 'verbfile_name')
        globals.AgentFileName = parser.get(
            'Dictionaries',
            'agentfile_name')
#		print "pc",globals.AgentFileName
        globals.DiscardFileName = parser.get(
            'Dictionaries',
            'discardfile_name')

        filestring = parser.get('Dictionaries', 'actorfile_list')
        globals.ActorFileList = filestring.split(', ')

        # otherwise this was set in command line
        if len(globals.TextFileList) == 0:
            if parser.has_option('Options', 'textfile_list'):  # takes priority
                filestring = parser.get('Options', 'textfile_list')
                globals.TextFileList = filestring.split(', ')
            else:
                ...
                while len(line) > 0:   # go through the entire file
                    ...
                    line = line.strip()
                    if len(line) > 0:
                        globals.TextFileList.append(line)
                    line = fpar.readline()
                fpar.close()


        if parser.has_option('Dictionaries', 'issuefile_name'):
            globals.IssueFileName = parser.get(
                'Dictionaries',
                'issuefile_name')

        if parser.has_option('Options', 'new_actor_length'):
            try:
                globals.NewActorLength = parser.getint(
                    'Options',
                    'new_actor_length')
            except ValueError:
                print(
                ...
                raise
        print("new_actor_length =", globals.NewActorLength)

        globals.StoponError    = get_config_boolean('stop_on_error')
        globals.WriteActorRoot = get_config_boolean('write_actor_root')
        globals.WriteActorText = get_config_boolean('write_actor_text')
        globals.WriteEventText = get_config_boolean('write_event_text')
        globals.NullVerbs  = get_config_boolean('null_verbs')
        globals.NullActors = get_config_boolean('null_actors')

        if parser.has_option(
                'Options', 'require_dyad'):  # this one defaults to True
            globals.RequireDyad = get_config_boolean('require_dyad')
        else:
            globals.RequireDyad = True

        # otherwise this was set in command line
        if len(globals.EventFileName) == 0:
            globals.EventFileName = parser.get('Options', 'eventfile_name')

        globals.CodeBySentence = parser.has_option(
            'Options',
            'code_by_sentence')
        print("code-by-sentence", globals.CodeBySentence)

        globals.PauseBySentence = parser.has_option(
            'Options',
            'pause_by_sentence')
        print("pause_by_sentence", globals.PauseBySentence)

        globals.PauseByStory = parser.has_option(
            'Options',
            'pause_by_story')
        print("pause_by_story", globals.PauseByStory)

        try:
            if parser.has_option('Options', 'comma_min'):
                globals.CommaMin = parser.getint('Options', 'comma_min')
            elif parser.has_option('Options', 'comma_max'):
                globals.CommaMax = parser.getint('Options', 'comma_max')
            elif parser.has_option('Options', 'comma_bmin'):
                globals.CommaBMin = parser.getint('Options', 'comma_bmin')
            elif parser.has_option('Options', 'comma_bmax'):
                globals.CommaBMax = parser.getint('Options', 'comma_bmax')
            elif parser.has_option('Options', 'comma_emin'):
                globals.CommaEMin = parser.getint('Options', 'comma_emin')
            elif parser.has_option('Options', 'comma_emax'):
                globals.CommaEMax = parser.getint('Options', 'comma_emax')
        except ValueError:
            print(
...
#	print "PTR-1:", theline,
    ...
    if ' ' not in theline:  # theline only contains a keyword
        globals.AttributeList = theline[1:-2]
#		print "PTR-1.1:", globals.AttributeList
        return
    pline = theline[1:].partition(' ')  # skip '<'
    globals.AttributeList = [pline[0]]
    theline = pline[2]
    while ('=' in theline):  # get the field and content pairs
        pline = theline.partition('=')
        globals.AttributeList.append(pline[0].strip())
        theline = pline[2]
        pline = theline.partition('"')
        if pline[2][0] == '"':   # twice-double-quote
            pline = pline[2][1:].partition('"')
            globals.AttributeList.append('"' + pline[0] + '"')
            theline = pline[2][1:]
        else:
            pline = pline[2].partition('"')
            globals.AttributeList.append(pline[0].strip())
            theline = pline[2]
#	print "PTR-2:", globals.AttributeList
...
# routine); if an error needs to be raised, use get_attribute()
    if (targattr in globals.AttributeList):
        return (
            globals.AttributeList[
                globals.AttributeList.index(targattr) + 1]
        )
    else:
        return ""
...
def read_discard_list(discard_path):
    ...
    logger.info("Reading " + globals.DiscardFileName)
    ...
    while len(line) > 0:  # loop through the file
        ...
        targ = targ[1:]
        list = globals.DiscardList.setdefault(prev, {})
        while targ != []:
...
def read_issue_list(issue_path):
    ...
    File is stored in globals.IssueList as a list of tuples (string, index) where
    index refers to the location of the code in globals.IssueCodes. The coding is done
    in check_issues()
    ...
    globals.IssueList = {}
    logger = logging.getLogger('petr_log')
    logger.info("Reading " + globals.IssueFileName)
    open_FIN(issue_path, "issues")

    globals.IssueCodes.append('~')  # initialize the ignore codes
    globals.IssueCodes.append('~~')
    ...
    while len(line) > 0:  # loop through the file
        ...
        if line[0] == '~':  # ignore codes are only partially implemented
            ...
            if code in globals.IssueCodes:
                codeindex = globals.IssueCodes.index(code)
            else:
                globals.IssueCodes.append(code)
                codeindex = len(globals.IssueCodes) - 1
	...
	ka = 0
	while ka < 128 :
		print globals.IssueList[ka],globals.IssueCodes[globals.IssueList[ka][1]]
		ka += 1
	"""
...
def read_verb_dictionary(verb_path):
    ...
    logger = logging.getLogger('petr_log')
    logger.info("Reading " + globals.VerbFileName)
    ...
    for line in file:
        ...
        if line.startswith("---"):
            ...
            for pat in resolve_synset(pattern):
                ...
                path =  globals.VerbDict['phrases'].setdefault(block_meaning,{})
                if not pre == ([],[]):
                    if pre[0]:
                        count = 1
            ...
            answer = segs[1].split()
            ev2 = pat
            path = globals.VerbDict['transformations']
            while len(ev2) > 1:
                source = ev2[0]
                verb = reduce(lambda a,b : a + b ,
                    map(lambda c: utilities.convert_code(globals.VerbDict['verbs'][c]['#']['#']['code'])[0] if not c== "Q" else -1,
                    ev2[-1].split("_")), 0)
...
#                print(stem)
    ...
    #print(sorted(globals.VerbDict['phrases'].keys()))
    #print(globals.VerbDict.__sizeof__())
    #print(globals.VerbDict['phrases'].__sizeof__())
    #exit()
    #for root , k in sorted(globals.VerbDict['verbs'].items()):
    #    print(root)
    #    print("\t",k)
...
def _read_verb_dictionary(verb_path):
    ...
    def make_phrase_list(thepat):
        ...
        while ka < len(phlist):
            if len(phlist[ka]) > 0:
                if (phlist[ka][0] == '&') and (
                        phlist[ka] not in globals.VerbDict):
                    print("WTF", phlist[ka])
                    print(sorted(globals.VerbDict.keys()))
                    exit()
...
def show_verb_dictionary(filename=''):
    ...
    if len(filename) > 0:
        ...
        fout.write('Run time: ' + globals.RunTimeString + '\n')

        for locword, loclist in globals.VerbDict.items():
...
def show_draft_verb_dictionary():
    """ temporary function until I can fix show_verb_dictionary"""
    fout = open("verb.dict.txt",'w')
    for k,v in sorted(globals.VerbDict.items()):
        fout.write(k + '\n')
        for i,j in v.items():
    ...
    while len(line) > 0:
        if line[0] == '[':  # Date
            ...
            if line[0] == '+':  # Synonym
                ...
                if globals.WriteActorRoot and len(current_acts) > 0:  # store the root phrase if we're only to use it
                    datelist.append(current_acts[0])
                for targ in current_acts:
                    list = globals.ActorDict
                    while targ != []:
                        if targ[0] in [' ', '']:
                            targ = targ[1:]
                            continue
...
def show_draft_actor_dictionary():
    fout = open("actor.dict.txt",'w')
    for j,k in sorted(globals.ActorDict.items()):
        fout.write(j + '\t\t' + str(k) + '\n')
    fout.close()
